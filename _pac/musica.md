---
author: rosepac
bootstrap: true
layout: post
title: "Sobre Música"
description: 'La música que me ha acompañado a lo largo de mi vida, todas mis canciones favoritas y mis gustos musicales preferidos.'
excerpt: 'La música que me ha acompañado a lo largo de mi vida, todas mis canciones favoritas y mis gustos musicales preferidos.'
date: 2020-11-03
last_modified_at: 2020-11-03 12:30:02
published: true
permalink: /musica/
canonical_URL: https://ciberninjas.com/musica/
thumbnail: /assets/img/2020/musica-chica.webp
feature-img: /assets/img/2020/musica-chica.webp
img: /assets/img/2020/musica-chica.webp
---


<div id = "loader"> </div>

<div id="content">
La música random del menda lerenda, rosepac. Vale. Mis gustos son eclépticos, por lo que vas a poder encontrarte casi de todo por entre estos reproductores, solamente espero que no te asustes.. Y puedas encontrar algo que te interese y entretenga. 😹

A lo largo de mi vida, siempre he escuchado más o menos la misma música, pero a veces se me ha ido la cabeza un poco más hacía el rock, y otras veces hacía la música románticona, e incluso he tenido mis ratos de mala ostia con la música punkarra y metalera.

Son unos añitos ya, que dan para haber ido escuchando casi de todo.

![La música Ciberninjas preferida que ha acompañado al menda Pablo Álvarez Corredera a lo largo de su vida, todas mis canciones favoritas y mis gustos musicales preferidos.](/assets/img/2020/musica-chica.webp)

<span id="menu"><strong>MENÚ MUSICAL RANDOM</strong></span>

- [**Mejores Canciones de Navidad 🎄🎅**](#mejores-canciones-de-navidad-)
- [**🤘 Rock Roll / Heavy**](#-rock-roll--heavy)
- [**🎸 Rock & Roll Clásico / Extranjero (en inglés)**](#-rock--roll-clásico--extranjero-en-inglés)
  - [Mi Lista de las Mejores Canciones del Rock Callejero](#mi-lista-de-las-mejores-canciones-del-rock-callejero)
- [**🎸 Rock Pop en Español**](#-rock-pop-en-español)
- [**📀 Los Mejores Discos del Rock Pop**](#-los-mejores-discos-del-rock-pop)
  - [**M-Clan: Sin Enchufe**](#m-clan-sin-enchufe)
- [**📢 Música de Cantautor**](#-música-de-cantautor)
  - [**Las Mejores Canciones de Antonio Vega**](#las-mejores-canciones-de-antonio-vega)
  - [**Las Mejores Canciones de Revolver**](#las-mejores-canciones-de-revolver)
  - [**Las Mejores Carlos Chaouen**](#las-mejores-carlos-chaouen)
- [**🌄 Los Mejores Acústicos**](#-los-mejores-acústicos)
  - [**80 Versiones en Acústico de Iratxo, El Niño de la Hipoteca, Andres Suárez, El Kanka y Más**](#80-versiones-en-acústico-de-iratxo-el-niño-de-la-hipoteca-andres-suárez-el-kanka-y-más)
  - [**87 Acústicos de Cantautores Españoles**](#87-acústicos-de-cantautores-españoles)
- [**🎤 Hip Hop**](#-hip-hop)
  - [**Las Mejores 195 Mejores Canciones del Hip Hop Español**](#las-mejores-195-mejores-canciones-del-hip-hop-español)
  - [**28 Mejores Temas del Chojín**](#28-mejores-temas-del-chojín)
  - [**Rapsusklei, Sharif, Rayden, ZPU, Nach**](#rapsusklei-sharif-rayden-zpu-nach)
  - [**Old School R&B Hip Hop Rap Soul**](#old-school-rb-hip-hop-rap-soul)
  - [**Las 32 Mejores Canciones de Eminem**](#las-32-mejores-canciones-de-eminem)
- [**Música Épica para para Gamers y Programadores**](#música-épica-para-para-gamers-y-programadores)
- [**💘 Pop / Love**](#-pop--love)
  - [**476 Canciones del Mejor Pop Rock Español**](#476-canciones-del-mejor-pop-rock-español)
  - [**26 Vídeos de Cantantes de Música Alternativa**](#26-vídeos-de-cantantes-de-música-alternativa)
  - [**21 Éxitos Latinos Románticones**](#21-éxitos-latinos-románticones)
  - [**6 Canciones de Música Alternativa con la que Enamorarte**](#6-canciones-de-música-alternativa-con-la-que-enamorarte)
  - [**Español 2020**](#español-2020)
- [**🎧 Relax & Trabajar**](#-relax--trabajar)
  - [**Chill Playlist (Billie Eilish, Lorde)**](#chill-playlist-billie-eilish-lorde)
  - [**Piano: Jose Luis Altamirano**](#piano-jose-luis-altamirano)
  - [**Playlist Nujabes / Chill Out / Relax**](#playlist-nujabes--chill-out--relax)
  - [**Smooth R&B 2019**](#smooth-rb-2019)
- [**🤠 R&B del 2000 al 2020**](#-rb-del-2000-al-2020)
  - [**R&B 2021 - 60 Mejores Canciones de R&B**](#rb-2021---60-mejores-canciones-de-rb)
  - [**459 Canciones de R&B**](#459-canciones-de-rb)
  - [**41 Canciones de R B del 2014**](#41-canciones-de-r-b-del-2014)
- [**🤠 Country**](#-country)
  - [Bluegrass Playlist](#bluegrass-playlist)
  - [**Playlist Country, Rock, Alternativa, Random**](#playlist-country-rock-alternativa-random)
  - [**Las 100 mejores canciones Country de 2010**](#las-100-mejores-canciones-country-de-2010)
  - [**Las mejores canciones Country 2021 / 2020**](#las-mejores-canciones-country-2021--2020)
  - [**Hot Country Playlist 2021 - New Hot Country Music 2021**](#hot-country-playlist-2021---new-hot-country-music-2021)
- [**🥳 Comercial Fiestera**](#-comercial-fiestera)
  - [**14 Canciones con las que darte una Fiesta 2010**](#14-canciones-con-las-que-darte-una-fiesta-2010)
- [**🤸‍♀️ Power Comercial**](#️-power-comercial)
- [**💲 Full Comercial**](#-full-comercial)
- [**Publicaciones Música**](#publicaciones-música)
  - [**Páginas web de música alternativa random**](#páginas-web-de-música-alternativa-random)

## **Mejores Canciones de Navidad 🎄🎅**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLz1LeT5FVsBsbCs8-Xbka-AvBJ8ASyJ9G" allowfullscreen></iframe>
</div><br />

## **🤘 Rock Roll / Heavy**

<br />
<div class="accordion" id="acordrock">
    <div class="card">
        <div class="card-header" id="rrOne">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link" data-toggle="collapse" data-target="#collapseOne">Extremoduro, marea, fito, eukz, saratoga, warcry, sinkope, la fuga</button>
            </h2>
        </div>
        <div id="collapseOne" class="collapse" aria-labelledby="rrOne" data-parent="#acordrock">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PL_MyavMOcxp0Ot65TsS1sQbNMF6KpZC1u" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="rrTwo">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseTwo">Söber, Mago y Leo</button>
            </h2>
        </div>
        <div id="collapseTwo" class="collapse" aria-labelledby="rrTwo" data-parent="#acordrock">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLBpmS6aVtOwvERCSrc0e-Fcan2KOZKsk4" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="rrThree">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree">Casi 700 vídeos del Mejor Heavy Metal</button>
            </h2>
        </div>
        <div id="collapseThree" class="collapse" aria-labelledby="rrThree" data-parent="#acordrock">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLglLEhldqRTN37zWaW6F3kPuFChsdcjvx" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="rrFour">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree">Grupos Asturianos. (rock,punk,ska,reggae,rap...)</button>
            </h2>
        </div>
        <div id="collapseThree" class="collapse" aria-labelledby="rrFour" data-parent="#acordrock">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLfW5xezkrsr6iUBjBNsfBK6EFHtTl-UX2" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="rrFive">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree">Lista Random con Buen Rock</button>
            </h2>
        </div>
        <div id="collapseThree" class="collapse" aria-labelledby="rrFive" data-parent="#acordrock">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=FLFeZtYtEE0yytSLuixMVffw" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="rrSix">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree">🎻 Full Hardcore con 2 Cellos</button>
            </h2>
        </div>
        <div id="collapseThree" class="collapse" aria-labelledby="rrSix" data-parent="#acordrock">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLtsjzWvKXet8gBs5MMsuNWQaOA52BQY4t" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
</div>
<br />

## **🎸 Rock & Roll Clásico / Extranjero (en inglés)**

<br />
<div class="accordion" id="acordrock">
    <div class="card">
        <div class="card-header" id="rrOne">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link" data-toggle="collapse" data-target="#collapseOne">3 Doors, Creed, Nickleback, Staind, Hinder, 3 Day's Grace, Linkin Park, Shinedown, etc..</button>
            </h2>
        </div>
        <div id="collapseOne" class="collapse" aria-labelledby="rrOne" data-parent="#acordrock">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLk2cJLvvpgA5GegHMQfRuKWWUn8CpCfXh" allowfullscreen></iframe>
</div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="rrTwo">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseTwo">48 Temazos del Rock en Inglés</button>
            </h2>
        </div>
        <div id="collapseTwo" class="collapse" aria-labelledby="rrTwo" data-parent="#acordrock">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLeyt4N2uorXsVZEuiLD_t54iI4PxkBRxX" allowfullscreen></iframe>
</div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="rrThree">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree">Casi 700 vídeos del Mejor Heavy Metal</button>
            </h2>
        </div>
        <div id="collapseThree" class="collapse" aria-labelledby="rrThree" data-parent="#acordrock">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLglLEhldqRTN37zWaW6F3kPuFChsdcjvx" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="rrFour">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree">Grupos Asturianos. (rock,punk,ska,reggae,rap...)</button>
            </h2>
        </div>
        <div id="collapseThree" class="collapse" aria-labelledby="rrFour" data-parent="#acordrock">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLfW5xezkrsr6iUBjBNsfBK6EFHtTl-UX2" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="rrFive">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree">Lista Random con Buen Rock</button>
            </h2>
        </div>
        <div id="collapseThree" class="collapse" aria-labelledby="rrFive" data-parent="#acordrock">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=FLFeZtYtEE0yytSLuixMVffw" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="rrSix">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree">🎻 Full Hardcore con 2 Cellos</button>
            </h2>
        </div>
        <div id="collapseThree" class="collapse" aria-labelledby="rrSix" data-parent="#acordrock">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLtsjzWvKXet8gBs5MMsuNWQaOA52BQY4t" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
</div>
<br />
<!-- 194 canciones del rock del mejor rock alternativo (-faltante-) -->
> 👷‍♂️ FALTANTES POR AÑADIR + 👷‍♂️**Crossover clásico**: 26 nuevas canciones.?.

### Mi Lista de las Mejores Canciones del Rock Callejero

[Listas de 20 Minutos](https://listas.20minutos.es/lista/mejores-canciones-del-rock-en-espanol-395047/){:target="_blank" rel="nofollow,noreferrer"} (En una época de Internet, esto era una moda.. Aunque no te lo creas +) 😹

## **🎸 Rock Pop en Español**

<br />
<div class="accordion" id="accordionExample">
    <div class="card">
        <div class="card-header" id="rpOne">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link" data-toggle="collapse" data-target="#collapseOne">Rock en español e inglés</button>
            </h2>
        </div>
        <div id="collapseOne" class="collapse" aria-labelledby="rpOne" data-parent="#accordionExample">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLA_I2ay5YcUXHaBmkpz0PdkdAlbs1IQTV" allowfullscreen></iframe>
                </div><br />
                <p>Jarabe de Palo, Vetusta Morla, Leiva, Marea, KITAI ft. Taburete, Funambulista, Fito & Fitipaldis, Muse..
                </p>
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="rpTwo">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseTwo">Las 200 mejores canciones de rock y pop de todos los tiempos</button>
            </h2>
        </div>
        <div id="collapseTwo" class="collapse" aria-labelledby="rpTwo" data-parent="#accordionExample">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PL979721712CED57BB" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="rpThree">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree">+700 Canciones de Rock Roll 219</button>
            </h2>
        </div>
        <div id="collapseThree" class="collapse" aria-labelledby="rpThree" data-parent="#accordionExample">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLRZlMhcYkA2FGdVe0qFuaCLHgT1EAXOvw" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
</div>
<br />

## **📀 Los Mejores Discos del Rock Pop**

### **M-Clan: Sin Enchufe**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PL4PEePVBJzU_ugtCft4eyMwibhqeoPSvF" allowfullscreen></iframe>
</div><br />

## **📢 Música de Cantautor**

### **Las Mejores Canciones de Antonio Vega**

👷‍♂️

### **Las Mejores Canciones de Revolver**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PL4204231F3C78EC32" allowfullscreen></iframe>
</div><br />

### **Las Mejores Carlos Chaouen**

👷‍♂️

## **🌄 Los Mejores Acústicos**

### **80 Versiones en Acústico de Iratxo, El Niño de la Hipoteca, Andres Suárez, El Kanka y Más**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLE_vhtPNDDNLBfzkoYIXVk8guN9snf9VO" allowfullscreen></iframe>
</div><br />

### **87 Acústicos de Cantautores Españoles**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLVT5ffxHjZQw2JnAyau2rUmFupGFVp-LP" allowfullscreen></iframe>
</div><br />

## **🎤 Hip Hop**

### **Las Mejores 195 Mejores Canciones del Hip Hop Español**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLp7o6ZCcsW2QALsLaLRmZyggrRPRm0L5Q" allowfullscreen></iframe>
</div><br />

### **28 Mejores Temas del Chojín**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLMnQ17spvsZwA3yEN4Pp19kKGAD7WbcXb" allowfullscreen></iframe>
</div><br />

### **Rapsusklei, Sharif, Rayden, ZPU, Nach**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLVXki0xOH49Z1HTRYbKaEMHb2JpmLLiLS" allowfullscreen></iframe>
</div><br />

### **Old School R&B Hip Hop Rap Soul**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PL_B89Ke8dejW62he5q7pRxGcTUdtNa394" allowfullscreen></iframe>
</div><br />

### **Las 32 Mejores Canciones de Eminem**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PL7E436F1EC114B001" allowfullscreen></iframe>
</div><br />

## **Música Épica para para Gamers y Programadores**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PL8M1frRRqO_qgc_TOTuvtaStPY00DfVHu" allowfullscreen></iframe>
</div><br />

## **💘 Pop / Love**

### **476 Canciones del Mejor Pop Rock Español**

<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLi5IanQRrFXULM1MSvLFVMnTGRVg8lJ2G" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

### **26 Vídeos de Cantantes de Música Alternativa**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLeFr_1Q-ltOmGHOQG_v5HeEocwV8JkVft" allowfullscreen></iframe>
</div><br />

### **21 Éxitos Latinos Románticones**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLWuSE8RjT0acdowwhELTVUTcWfaMSxRqb" allowfullscreen></iframe>
</div><br />

### **6 Canciones de Música Alternativa con la que Enamorarte**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLKqlXOh4C5eCQD_owrZPDrHJRWJgkHgrr" allowfullscreen></iframe>
</div><br />

### **Español 2020**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLSYbV1H4VkCeCiimiKQ7VijAAHiDoE134" allowfullscreen></iframe>
</div><br />

## **🎧 Relax & Trabajar**

### **Chill Playlist (Billie Eilish, Lorde)**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PL3GoK8WI2eIG_i2RdIdJLuoigFb7fkWP5" allowfullscreen></iframe>
</div><br />

### **Piano: Jose Luis Altamirano**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLeLQcPuzskwKcWR4pA7su8dkQfyjhSHGJ" allowfullscreen></iframe>
</div><br />

### **Playlist Nujabes / Chill Out / Relax**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PL8M1frRRqO_qTehLIJjj_bHIjLOpenbvS" allowfullscreen></iframe>
</div><br />

### **Smooth R&B 2019**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLdt0ehPLKLZFOfaLU8D8-UrvpGQt_6Jop" allowfullscreen></iframe>
</div><br />

## **🤠 R&B del 2000 al 2020**

### **R&B 2021 - 60 Mejores Canciones de R&B**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLRZlMhcYkA2FGdVe0qFuaCLHgT1EAXOvw" allowfullscreen></iframe>
</div><br />

### **459 Canciones de R&B**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLFbWuc6jwPGeqFkoDBq87CcmlurwrlEGv" allowfullscreen></iframe>
</div><br />

### **41 Canciones de R B del 2014**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLPtC92Qym_RiiWb3nE4wYlb_KgtC_qbga" allowfullscreen></iframe>
</div><br />

## **🤠 Country**

### Bluegrass Playlist

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLKUA473MWUv2mddNMPh-MJkgTR5AjTpl3" allowfullscreen></iframe>
</div><br />

### **Playlist Country, Rock, Alternativa, Random**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PL8M1frRRqO_rgKXsh5WOV3AqP_Sj0X6o7" allowfullscreen></iframe>
</div><br />

### **Las 100 mejores canciones Country de 2010**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/DQYNM6SjD_o" allowfullscreen></iframe>
</div><br />

### **Las mejores canciones Country 2021 / 2020**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/Y2E71oe0aSM" allowfullscreen></iframe>
</div><br />

### **Hot Country Playlist 2021 - New Hot Country Music 2021**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLgFPSBWI2ATuCRZ1G6mxkqeSShDPOhtiF" allowfullscreen></iframe>
</div><br />

## **🥳 Comercial Fiestera**

### **14 Canciones con las que darte una Fiesta 2010**

<div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PL9BC5F01ED0049D0D" allowfullscreen></iframe>
</div><br />

## **🤸‍♀️ Power Comercial**

<br />
<div class="accordion" id="acordPowerComercial">
    <div class="card">
        <div class="card-header" id="pcOne">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link" data-toggle="collapse" data-target="#collapseOne">Playlist para hacer Deporte 2020</button>
            </h2>
        </div>
        <div id="collapseOne" class="collapse" aria-labelledby="pcOne" data-parent="#acordPowerComercial">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube.com/embed/videoseries?list=PLA_I2ay5YcUUr8_rE-DlVknObkJ6-Rn3q" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="pcTwo">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseTwo">Clásicos de Tito El Bambino, 75 Vídeos</button>
            </h2>
        </div>
        <div id="collapseTwo" class="collapse" aria-labelledby="pcTwo" data-parent="#acordPowerComercial">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLB32D28D087F3CF4B" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="pcThree">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree">Clásicos del Reagettonto, 134 Vídeos</button>
            </h2>
        </div>
        <div id="collapseThree" class="collapse" aria-labelledby="pcThree" data-parent="#acordPowerComercial">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLZEemm-siVWoypkeT50V6gZgmuttI9i3j" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="pcFour">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree">Clásicos del Reagettonto, del 2020 al 2010</button>
            </h2>
        </div>
        <div id="collapseThree" class="collapse" aria-labelledby="pcFour" data-parent="#acordPowerComercial">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLcoqIQllPxWzoCqo3Srp_vV0-qNFKhUgV" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
</div>

<br />

## **💲 Full Comercial**

<br />
<div class="accordion" id="acordFullComercial">
    <div class="card">
        <div class="card-header" id="fcOne">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link" data-toggle="collapse" data-target="#collapseOne">Full Comercial 2021</button>
            </h2>
        </div>
        <div id="collapseOne" class="collapse" aria-labelledby="fcOne" data-parent="#acordFullComercial">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLyORnIW1xT6waC0PNjAMj33FdK2ngL_ik" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="fcTwo">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseTwo">Full Comercial 2017</button>
            </h2>
        </div>
        <div id="collapseTwo" class="collapse" aria-labelledby="fcTwo" data-parent="#acordFullComercial">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PL8M1frRRqO_rnt1TrIlCug4kY_ARs2KEy" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="fcThree">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree">Full Comercial / Reggae-tonto 2021 / 2020</button>
            </h2>
        </div>
        <div id="collapseThree" class="collapse" aria-labelledby="fcThree" data-parent="#acordFullComercial">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLq3UZa7STrbrXuQZlIM_4VQSiiNK3YDf6" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="fcFour">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree">*18 Canciones del regaetton de hace años</button>
            </h2>
        </div>
        <div id="collapseThree" class="collapse" aria-labelledby="fcFour" data-parent="#acordFullComercial">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLptG5zt9Oqs6tsWy-Ko7vGV73Q8JM8gm1" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header" id="fcFive">
            <h2 class="mb-0">
                <button type="button" class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree">Full Comercial / Reggae-tonto de Siempre</button>
            </h2>
        </div>
        <div id="collapseThree" class="collapse" aria-labelledby="fcFive" data-parent="#acordFullComercial">
            <div class="card-body">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLo3pNg0eiPc8rRO3XqFnl1Je2GTu7am-F" allowfullscreen></iframe>
                </div><br />
            </div>
        </div>
    </div>
</div>

**¡Próximamente Más!**

## **Publicaciones Música**

- [Mi Blog de Canciones Random](https://pabloalvarezcorredera.blogspot.com/){:target="_blank"} (😿 Abandonadito.)

### **Páginas web de música alternativa random**

1. [MixCurb](https://mixcurb.com/){:target="_blank" rel="nofollow,noreferrer"}
2. [ForeignRAP](https://foreignrap.com/){:target="_blank" rel="nofollow,noreferrer"}: Página web que te permite escuchar "variados" de "hip-hop" bajo un aspecto retro, y seccionado por países..
3. [10 canciones de rap español, para dedicar](https://www.guaridahiphop.com/2016/03/top-10-canciones-de-rap-para-dedicar-en.html?m=1){:target="_blank" rel="nofollow,noreferrer"}

<!-- CARGAR LAS LIBRERÍAS DE BOOTSTRAP Extraído de https://www.tutorialrepublic.com/codelab.php?topic=bootstrap&file=accordion -->
<script src="https://code.jquery.com/jquery-3.5.1.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.1/dist/umd/popper.min.js"></script>
</div>